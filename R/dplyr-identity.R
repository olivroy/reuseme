...
#' @rdname identity
#' @export
slice_min_max_identity <- function(x, order_by, each = FALSE, ..., n, prop, by = NULL, with_ties = TRUE, na_rm = FALSE, nrows = NULL, extra_msg = NULL) {
  if (!rlang::is_interactive()) {
    return(invisible(x))
  }
  res <- slice_min_max(
...
#' @rdname identity
#' @export
slice_group_sample_identity <- function(x, n_groups = 1, group_var = NULL, nrows = NULL, extra_msg = NULL) {
  if (!rlang::is_interactive()) {
    return(invisible(x))
  }
