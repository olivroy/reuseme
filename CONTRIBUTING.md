...
This outlines how to propose a change to reuseme. For a detailed
...
- `define_outline_criteria()` and underlying `o_is_*()` for changing
  logic around what constitutes an outline element

- `define_outline_criteria()` if an item shows as outline, but seems
...
- `display_outline_element()` if the outline element doesn’t show as
  expected (i.e right position, but not enough or too much stripping),
  edit
...
Example with [ggtitle](https://olivroy.github.io/reuseme/link)

1.  Create a new function in `R/outline-criteria.R` (place right before
...
Be careful for markdown vs non-markdown (i.e. section title not the
same)
...
4.  Create new conditions on how to display in
...
6.  Look at the result. Ideally, add to \_outline/my-analysis.R, so it
    shows somehow in snapshots.
